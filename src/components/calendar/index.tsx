...
export const Calendar = ({ config }: CalendarProps) => {
	...
	const holidays = useMemo(() => config?.holidays || [], [config?.holidays])
					...
					<div className="grid grid-cols-7 gap-1">
						{days.map((day, index) => {
							const isTodayHoliday =
								day.isToday && (day.isHoliday || day.isWeekend)
							const isTodayRegular =
								day.isToday && !day.isHoliday && !day.isWeekend
							...
							return (
								<div
									...
									className={cn(
										...
										isTodayHoliday &&
											'bg-red-500/40 font-bold text-red-100 is-today-holiday',
										isTodayRegular &&
											'bg-white/20 font-bold text-white is-today-regular',
										day.isHoliday &&
											!day.isToday &&
											'bg-red-500/30 text-red-200 is-holiday-but-not-today',
										day.isWeekend &&
											!day.isHoliday &&
											!day.isToday &&
											'bg-red-500/15 text-red-300 is-weekend-but-not-today-and-not-holiday',
										!day.isHoliday &&
											!day.isWeekend &&
											!day.isToday &&
											'text-white is-not-holiday-and-not-weekend-and-not-today'
									)}
								>
									{format(day.date, 'd')}
								</div>
