...
/**
 * Calculate the progress through the current period (day or night)
 * Returns a value between 0-1 representing progress through the current period
 */
const getPeriodProgress = (sunrise: number, sunset: number): { progress: number; isDaytime: boolean } => {
  const now = Date.now() / 1000
  ...
  // We're in nighttime
  // Calculate night period length (from sunset to next sunrise, which is +24h)
  const nextSunrise = sunrise + 86400 // Add 24 hours
  let nightStart = sunset
  let nightEnd = nextSunrise

  // If we're before today's sunrise, we're in the previous night
  if (now < sunrise) {
    nightStart = sunset - 86400 // Previous sunset
    nightEnd = sunrise
  }

  const nightLength = nightEnd - nightStart
  const timeSinceNightStart = now - nightStart
  return {
    progress: timeSinceNightStart / nightLength,
    isDaytime: false
  }
}
